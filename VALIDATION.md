...
2026-09-26：**尚未达到正式发行条件。** 以下是本轮新证据；后文保留历史预览版记录。各组不合并为“最终通过总数”，机器可读结果见 [validation-results.json](validation-results.json)。历史原始明细以对应预览发行包为准，当前 JSON 同时保留 preview.7 的原报告。
...
Stable 官方通道当前指向 2.0.0；Beta 基线为 2.2.0-beta.1。两版官方 SDK 均在 `sdk/index.ts(22,8)` 因缺失 `./extension-inspector` 报 TS2307，见 [复现记录](maintenance/SDK-BLOCKER.md)。npm 安装已成功；严格构建在类型检查处停止，没有生成本轮工坊审核包，没有修改 SDK 或降低清单声明来绕过。

首次沙箱 WPF 遇到附属状态写入限制，保留了失败记录与原数据；两种 PowerShell 在全新隔离目录、正常权限下重跑通过。首次 npm 下载连接重置，重试成功。这里记录失败原因和后续证据，不抹去失败尝试。

尚未通过：双宿主同包安装／打开／操作、合成剧情存读档与 Windows 导出、三款客户端完整模型任务及更新验证、原生安装器点击／取消／文件夹选择与缩放、最终工坊产物许可和源码映射检查、最终工坊预检、正式发布后的匿名下载安装及工坊投稿。步骤见 [发行门槛](maintenance/RELEASE-GATES.md)。不得将当前候选改标正式版或提交工坊审核。
