...
格式依据：[官方剧本 JSON 参考](https://docs.avg-engine.com/reference/script-json)，2026-09-25 复核。这里提供有限的写作入口，不是完整 schema。改动前重新核对目标版本；保留已有未知字段。
...
运行 `scripts/check_project.py` 检查有限的结构、索引和本章片段引用；它会标明未知指令未验证。默认 auto 对官方兼容的旧分支省略 mode 按 jump 检查并提示，不回写原文件；显式 fragments 模式按新建约定要求 mode。缺省与显式错误值不同，不把 null 或未知 mode 当默认值。然后人工核对变量、角色、资产、参数语义、蓝图和玩家流程。两条路线都要预览；分支片段一般调用后返回，不天然构成互斥结局，见 [Branch 官方行为](https://docs.avg-engine.com/manual/writing/blocks/branch)。
